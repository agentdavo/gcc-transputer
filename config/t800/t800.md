;; GCC machine description for IMS T800 transputer
...
(define_expand "movsi"
  ...
  ""
  "{
    /* T800 can move to memory only from a general reg */
    ...
    /* No special care is taken for the case when either of operands
       ...
       is really a stack slot).  We're sure the default pattern we
...
(define_expand "movhi"
  ...
  "{
	...
	/* Storing HImode.  On t800, we have to do this by shifts and
	   byte stores. */
...
(define_expand "movhi"
  ...
  ""
  "{
    /* T800 can move to memory only from a reg */
    if (GET_CODE (operands[0]) == MEM)
      {
        if (! reg_p (operands[1]))
          {
        ...
        if (REG_P (operands[0]))
          {
            ...
            emit_move_insn (t800_force_nonlocal (temp), operands[1]);
...
/* {symbol,label}_ref's are not loaded by this, since it would yield an
...
/* This insn is only generated by reg-stack converter. */
...
/* Unlike the hardware ldlp, this pattern takes an arbitrary offset
   and handles it with an additional adc, if needed. So QImode for
   operand 1.  */
...
/* QImode pseudos get word-wide stack slot allocated, so we can use
   SImode insns which don't need scratch registers. */

;(define_expand "reload_outqi"
;  [(set (match_operand:QI 0 "local_operand" "=m")
;        (match_operand:QI 1 "ABCreg_operand" "r"))
;   (clobber (match_operand:SI 2 "" "=&r"))]
;  ""
;  "{
;    /* Emit ldlp to load byte's address into the scratch reg */
...
#ifdef HAVE__ss
/* Should be HAVE__ssub, but we don't have a separate insn for `ssub' */
...
/* `diff' and `sub' are both short, and both take 1 cycle.  Prefer
   `diff' for its not setting the error flag...  */
...
;; tstM
;;------------------------------
;; Should not be defined as T800 does not have (cc0)
...
/* Insns of this group on T800 are only able to take input from
...
/* Insns of this group on T800 are only able to output to memory.
...
/* Note that RTL code <fix:xF> represents a "round-to-zero"
   conversion.  Therefore we should override default rounding mode of
   transputer, which is "round-to-nearest".  */
...
(define_insn "extendqisi2"
  [(set (match_operand:SI 0 "ABCreg_operand" "=a")
        (sign_extend:SI (match_operand:QI 1 "ABCreg_operand" "0")))
   (clobber (match_scratch:SI 2 "=&r"))]
  ""
  "ldc 0x80\;xword")
...
/* I think we can go with general_operand in insns which load
   something onto a regstack: reload should be able to use Areg (which
...
/* disabled: gcc will generate default conversion code that will not
   contain that obligatory `rev' at least */
...
/* Used to have (gt:SF ...) here, but it would produce unrecognizable
   insn for gen__fpgt (result, DF_operand1, DF_operand2)  */
...
/* _gcall_aggregate is a variation of normal _gcall used for models
   ...
   This should come ahead of normal `gcall'; t800_expand_call puts
   CCmode on CALL rtx when it wants this pattern to match.  */
...
(define_insn "_gcall_aggregate"
  ...
  T800_OUTPUT__GCALL;
#else
  return which_alternative ? \"ajw -1\;ldc 2\;ldpi\;stl 0\;gcall\;ajw -3\"
...
;; casesi
;;------------------------------
;; tablejump is used instead; but this may change if we decide to have
;; jump instructions in table rather than addresses/offsets.
...
(define_insn "tablejump"
  [(set (pc) (plus:SI (pc) (match_operand:SI 0 "ABCreg_operand" "a")))
   (use (label_ref (match_operand 1 "" "")))
   (clobber (match_dup 0))]
  ""
  "*{
#if T800_AS_ADC_LABELDIFF_OK
  return \"adc %1-LF%=\;ldpi\\nLF%=:\;gcall\";
#else
  /* OK, after all we know the table starts right after what we emit here,
     and gcall occupies 1 byte. */
  return \"adc 1\;ldpi\;gcall\";
#endif
}")
